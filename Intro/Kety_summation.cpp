...
int main(){
   char		/* Initialize at valid loop driver value */
   		runAgain = 'y';	
   const int	/* The power of x in the function f(x) = x^5 + 10 */
      		FUNCTION_POWER = 5,
		/* The addition of 10 in the function f(x) = x^5 + 10 */
		FUNCTION_INCREASE = 10;
   int		/* Controls if calculation will be for- or while-loop, 
		   based on user input, initialized to 1 (for-loop) */	
		forOrWhile = 1,
		/* Initializes loop driver for while-loop calculation method */
		whileLoopDriver = 1,
		/* Initializes loop driver for for-loop calculation method */
		forLoopDriver = 1,
