...
int main(){
    ...
    while((runAgain == 'y') || (runAgain == 'Y')){
        ...
        while((!(cin >> firstNum >> secondNum)) || (firstNum < 0) || 
              (secondNum < 0)){
        ...
        */
        ...
        }while((!(is_positive_int(numString1))) || (!(is_positive_int(numString2))));
...
void get_positive_int(string& numString1, string& numString2){
    ...
    for(int i = 0; i < userString.size(); i++){
        ...
        if((charValue < DECIMAL_CHAR_MIN) || (charValue > DECIMAL_CHAR_MAX)){
            invalidInput++;
        }
