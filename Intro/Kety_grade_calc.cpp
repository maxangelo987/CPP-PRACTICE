 ...
 *	File Name:	Kety_grade_calc.cpp
 *	Description:	For a number of assignments input from the user, sum 
 *			the points earned and points possible from each 
...
int main() {
   ...
   /* Loop preventing non-number and negative input to numberOfAssignments 
      via cin */
   do{
   ...
   for(
	...
	currentAssignment++){
      /* Loop preventing non-number, negative, and zero input to pointsPossible 
	 via cin */
      do{
	 ...
	 if(!(validResponse)){
      ...
      /* Loop preventing non-number, negative numbers, and numbers greater  
	 than pointsPossible */
      do{
   ...
   /* Calculation to the hundredths and compensating for floating-point 
      precision */
   ...
   if 	(
	 (percentageOfPointsPossible + 0.5) >= 
	 (((int)(percentageOfPointsPossible)) + 1)
	){
