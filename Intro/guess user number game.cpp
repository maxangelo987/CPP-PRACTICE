...
int main() {
    ...
    const int 	inaccurateResponse = 1,	/* Each inaccurate response is equivalent to 1 strike */
                fairGame = 0;	/* Number of inaccurate responses in a fair game */
    ...
    while(runAgain == 'y'){
        ...
        } while ((!(validResponse)) || (randomInt < guessFloor) || 
        ...
        for (guessCount = 1; guessCount < 6; guessCount++){
            ...
            if (!(userVerifies == 'y')){	
                ...
                do{
                    ...
                    else if ((guessVariance == guessLow) && (!(currentGuess < randomInt))){
                        strikes -= inaccurateResponse;
                        if (currentGuess == randomInt)
                            strikes -= exitGuessCountLoop;		//Ensures strikes < fairGame
                    }
                    else if ((guessVariance == guessPerfect) && (!(currentGuess == randomInt)))
                        strikes -= exitGuessCountLoop;		//Ensures strikes < fairGame
                    else if ((guessVariance == guessHigh) && (!(currentGuess > randomInt))){
                        strikes -= inaccurateResponse;
                        if (currentGuess == randomInt)
                            strikes -= exitGuessCountLoop;		//Ensures fairGame is negative
                    }
                } while (!(validResponse) || (guessVariance < guessLow) || (guessVariance > guessHigh));
            }
            ...
            switch (guessVariance){
                case(1):		//Guess is too low
                    ...
                    /* Ends program if user guess verification is inaccurate */
                        ...
                        cout << "If you're not going to play fair, " << "I don't want to play.\n\n";
